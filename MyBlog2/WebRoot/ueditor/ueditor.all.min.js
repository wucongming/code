...
"brush:"+d+";toolbar:true;";else{var h="";c.collapsed?h=r.ie?8<r.version?"":"&nbsp;":"<br/>":(b=c.extractContents(),c=this.document.createElement("div"),c.appendChild(b),p.each(UE.filterNode(UE.htmlparser(c.innerHTML.replace(/[\r\t]/g,"")),this.options.filterTxtRules).children,function(a){r.ie&&8<r.version?("element"==a.type?"br"==a.tagName?h+="\n":w.$empty[a.tagName]||(p.each(a.children,function(b){"element"==b.type?"br"==b.tagName?h+="\n":w.$empty[a.tagName]||(h+=b.innerText()):h+=b.data}),/\n$/.test(h)||
